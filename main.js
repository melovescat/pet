...
const DEFAULTS = Object.freeze({
  ...
  currentFrame: 0,
  flip: false,
  // New Hand Control Defaults
  handScale: 1.0, // Scale factor for the hand sprite
  handYOffset: 0, // Vertical pixel offset for the hand sprite
});
...
(() => {
  ...
  const PetPetAnimation = (canvas, hand, sprite) => {
    ...
    const renderFrame = (_frame, _ctx, _adjust) => {
      ...
      // Hand dimensions and position
      const handDw = OUT_SIZE * g.handScale;
      const handDh = OUT_SIZE * g.handScale;
      const handDx = OUT_SIZE / 2 - handDw / 2; // Center the hand
      
      // Calculate hand Dy based on sprite position, handYOffset, and hand scale.
      // The original formula is a bit cryptic, so we apply the new controls on top of it.
      const baseHandDy = Math.max(0, ~~(cf.dy * 0.75 - Math.max(0, g.spriteY) - 0.5));
      const handDy = baseHandDy + g.handYOffset; 
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    const reset = () => {
      ...
      $("#handScale").value = DEFAULTS.handScale * 100; // New input
      $("#handYOffset").value = DEFAULTS.handYOffset; // New input
    ...
    ["input", "change"].forEach((event) => {
      ...
      // Change hand size (New control)
      $("#handScale").addEventListener(
        event,
        (e) => {
          const newHandScale = (clamp(parseInt(e.target.value), 50, 300) / 100).toFixed(3);
          if (g.handScale !== newHandScale) {
            g.handScale = newHandScale;
            animation.tick();
          }
        },
        { passive: true, capture: true }
      ...
      // Change hand Y offset (New control)
      $("#handYOffset").addEventListener(
        event,
        (e) => {
          const newYOffset = clamp(parseInt(e.target.value), -50, 50);
          if (g.handYOffset !== newYOffset) {
            g.handYOffset = newYOffset;
            animation.tick();
          }
        },
        { passive: true, capture: true }
    ...
    const $downloadBtn = $("#downloadGif"); // New download button
    let lastBlob = null; // Store the last generated blob for download
    ...
    const renderer = GifRenderer(
      ...
      (startTime) => {
        ...
        $downloadBtn.disabled = true; // Disable download during render
    ...
    // Download button handler (New functionality)
